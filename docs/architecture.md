...
apps/analytics  Python 3.11 FastAPI service (uv-managed)
...
scripts/        dev-db.sh — project-local PostgreSQL 16 (no Docker needed)
```

Local run: `scripts/dev-db.sh init` → `pnpm seed` → `pnpm dev` (web on :3000)
→ `uv run uvicorn ok_analytics.main:app --port 8000` (analytics). See README.
...
| Panel & consent | panelists, custom_fields, panelist_attributes, consent_records, tags, panelist_tags, panelist_notes, segments, contact_events, import_batches | PII lives only here; anonymization scrubs identity and unlinks responses |
...
for reproducibility. Numerical correctness is guarded by 32 pytest fixtures
