...
Updated: 2026-07-21 (Phase A remediation reviewed locally)
...
| 4 | Panel list/profile/segments/governance + import workflow | ⚠️ partial — synthetic data only until ADR-006 consent mapping is decided and implemented |
...
4. **Import (synthetic data only)** — CSV/XLSX wizard with preview, auto-mapping, validation, dedup
   (external_id/email/none), consent confirmation, dry run, commit, audit
   events, downloadable error report; verified end-to-end in Chromium. Real-data consent provenance is not implemented (ADR-006).
...
11. **Statistics vs fixtures** — 36 pytest cases: hand-computed ANOVA
...
## Test inventory at Phase A head

- `packages/domain`: 31 Vitest tests (metrics, logic engine, permissions, sampling, submission)
- `apps/web`: 40 Vitest tests (8 RLS tenant isolation, 5 import validation, 1 import parse, 26 Phase A environment/audience/analytics tests)
- `apps/web` e2e: 8 Playwright specs (roles, panel, closed loop, analytics)
- `apps/analytics`: 36 pytest tests (numerical fixtures + export round-trips)
...
## Phase B addendum: PGlite local development (2026-07-21)

Implemented: PGlite (PostgreSQL WASM) as the permanent local development
database — `scripts/dev-db.mjs` manager, `LOCAL_DATABASE_ENGINE` switch,
`SET LOCAL ROLE cx_app` RLS mitigation in PGlite mode, pure-JS seed runner
(`scripts/run-ts.mjs`, replaces tsx), migration ledger with SHA-256 checksums,
26 manager tests + 3 RLS-role tests, extended env tests, CI pinned to the
authoritative native PostgreSQL gate. See
`docs/adr/008-pglite-local-development.md` and CHANGELOG. Remaining verified
gap: Playwright E2E not executed in the implementation sandbox (browser
download blocked); run locally/CI.
