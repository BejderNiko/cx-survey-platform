...
// Engine-aware default from lib/env: PGlite (postgres://…/postgres) or native
// (postgres://…/cx_platform). An explicit DATABASE_ADMIN_URL always wins.
const adminUrl = process.env.DATABASE_ADMIN_URL ?? env.databaseAdminUrl;

/**
 * Destructive-seed safety: this seed wipes and recreates the seeded
 * organizations, so it must never run against hosted staging/production by
 * accident. It only accepts loopback database hosts and refuses to run in
 * hosted Vercel runtimes. There is deliberately no override flag.
 */
function assertLocalSeedTarget(url: string): void {
  if (process.env.VERCEL && ["production", "preview"].includes(process.env.VERCEL_ENV ?? "")) {
    throw new Error("Refusing to seed: the deterministic seed must never run in a hosted Vercel environment.");
  }
  let hostname: string;
  try {
    hostname = new URL(url).hostname.toLowerCase();
  } catch {
    throw new Error("Refusing to seed: DATABASE_ADMIN_URL is not a parseable URL.");
  }
  const loopback =
    hostname === "localhost" || hostname === "::1" || hostname === "[::1]" || hostname.endsWith(".localhost") || hostname.startsWith("127.");
  if (!loopback) {
    throw new Error(
      "Refusing to seed: DATABASE_ADMIN_URL does not point at a loopback address. " +
        "The destructive deterministic seed is restricted to local development and CI databases.",
    );
  }
}
assertLocalSeedTarget(adminUrl);
