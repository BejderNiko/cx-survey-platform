...
// Resolved lazily so importing this module during `next build` never reads
// runtime-only environment (see lib/env.ts, F5-001).
let cachedSecret: Uint8Array | undefined;
function sessionSecret(): Uint8Array {
  return (cachedSecret ??= new TextEncoder().encode(env.sessionSecret));
}
