...
/** Hard ceiling on any single analytics HTTP call (F5-006); overridable for tests. */
function analyticsTimeoutMs(): number {
...
async function call(path: string, init: RequestInit): Promise<Response> {
  ...
  const timeoutMs = analyticsTimeoutMs();
  try {
    res = await fetch(`${env.analyticsUrl}${path}`, {
      ...init,
      headers,
      // Never block the calling request (and, previously, a DB transaction)
      // indefinitely on a slow or hung analytics service.
      signal: init.signal ?? AbortSignal.timeout(timeoutMs),
    });
  } catch (e) {
    if (e instanceof DOMException && e.name === "TimeoutError") {
      throw new AnalyticsServiceError(
        `The analytics service did not respond within ${timeoutMs / 1000}s.`,
        504,
      );
    }
    throw new AnalyticsServiceError(
