...
export default async function FeatureRequestsPage() {
  ...
  const data = await withUser(session.userId, session.orgId, async (tx) => {
    const [requests, events, members] = await Promise.all([
      tx`select fr.id, fr.title, fr.description, fr.status, fr.owner_id, fr.source_path, fr.section, fr.target_snapshot, fr.created_at, fr.updated_at, creator.full_name as creator, owner.full_name as owner_name from feature_requests fr join users creator on creator.id = fr.created_by left join users owner on owner.id = fr.owner_id where fr.org_id = ${session.orgId} order by fr.updated_at desc limit 200`,
    ...
    const missing = error && typeof error === "object" && "code" in error && (error.code === "42P01" || error.code === "42703");
