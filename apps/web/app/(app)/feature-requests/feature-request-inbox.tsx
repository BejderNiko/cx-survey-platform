...
type Row = { id: string; title: string; description: string; status: string; ownerId: string | null; ownerName: string | null; creator: string; updatedAt: string; createdAt: string; sourcePath: string | null; section: string | null; targetSnapshot: Record<string, unknown>; events: { id: string; type: string; actor: string | null; at: string; details: Record<string, unknown> }[] };
...
export function FeatureRequestInbox({ rows, members, canManage }: { rows: Row[]; members: { id: string; name: string }[]; canManage: boolean }) {
  ...
  const yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1); const previousDay = rows.filter((row) => new Date(row.createdAt).toDateString() === yesterday.toDateString());
  return <div className="space-y-4">
    ...
    {previousDay.length > 0 && <p className="text-sm font-semibold text-heading">Feedback fra i går: {previousDay.length}</p>}
    {rows.map((row) => <Card key={row.id} title={<span className="flex items-center gap-2"><Badge>{row.status}</Badge>{row.title}</span>}>
      <p className="whitespace-pre-wrap text-sm">{row.description}</p><p className="mt-2 text-xs text-muted">Oprettet af {row.creator} · opdateret {row.updatedAt}{row.sourcePath ? ` · ${row.sourcePath}` : ""}{row.section ? ` · ${row.section}` : ""}</p><details className="mt-2 text-xs"><summary className="cursor-pointer">Valgt UI-element</summary><pre className="mt-1 overflow-auto rounded bg-background p-2">{JSON.stringify(row.targetSnapshot, null, 2)}</pre><button type="button" className="mt-1 underline" onClick={() => navigator.clipboard.writeText(`Implementér feedback: ${row.description}\nSide: ${row.sourcePath ?? "ukendt"}\nSektion: ${row.section ?? "ukendt"}\nElement: ${JSON.stringify(row.targetSnapshot)}`)}>Kopiér AI-prompt</button></details>
