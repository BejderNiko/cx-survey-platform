...
/**
 * Run an analysis without holding a DB transaction across the analytics HTTP
 * call (F5-006). Sequence:
 *   1. Authorize, load the scoped dataset, create the recipe, and commit a
 *      durable `analysis_runs` row with status 'running' (transaction closes).
 *   2. Call the analytics service OUTSIDE any transaction, with a timeout.
 *   3. Open a fresh authorized transaction to mark the run succeeded/failed,
 *      scoped by org so one tenant can never finalize another tenant's run.
 * A process crash between steps 1 and 3 leaves a durable 'running' row for
 * crash reconciliation (see docs/adr/004) rather than losing the record.
 */
export async function runAnalysis(input: RunAnalysisInput): Promise<{
  ...
  const prepared = await withAuthorized("analytics.run", async (tx, session) => {
    ...
    return { runId: run.id as string, orgId: session.orgId as string, payload };
  });

  // Step 2 — analytics call with no DB transaction open and an explicit timeout.
  let result: AnalysisResultPayload;
  try {
    result = await runAnalysisRemote({
      procedure: input.procedure,
      params: input.params,
      seed: input.seed ?? null,
      dataset: prepared.payload,
    });
  } catch (e) {
    const message = e instanceof AnalyticsServiceError ? e.message : "Analysis failed.";
    await withAuthorized("analytics.run", async (tx, session) => {
      const updated = await tx`
        update analysis_runs set status = 'failed', error = ${message}, finished_at = now()
  ...
  // Step 3 — finalize in a new transaction, org-scoped so the update can only
  // ever touch this tenant's own run row. Keep this outside the service-call
  // catch: a database finalization failure must not be mislabeled as an
  // analytics-service failure.
  await withAuthorized("analytics.run", async (tx, session) => {
    const updated = await tx`
      update analysis_runs set status = 'succeeded', results = ${tx.json(result as never)},
