...
import { randomSample, segmentDefinition } from "@ok/domain";
import { withAuthorized } from "@/lib/auth";
import { audit } from "@/lib/audit";
import { env } from "@/lib/env";
import { applyGovernance, getGovernance, listPanelistIds } from "@/lib/data/panel";
...
export async function createPanelInvite(input: InviteInput) {
  const result = await withAuthorized("distributions.create", async (tx, session) => {
    ...
    const candidates = await listPanelistIds(tx, { segment, lifecycle: "active" });
    ...
    if (input.method === "random" && input.sampleSize && input.sampleSize < eligible.length) {
      selected = randomSample(eligible, input.sampleSize, seed).selected;
    }
    ...
    const [dist] = await tx`
      ...
      values (${session.orgId}, ${input.studyId}, ${version.id}, 'panel_invite', ${input.name || "Panel invitation"},
              ${tx.json({
                method: input.method, seed: input.method === "random" ? seed : null,
    ...
    await audit(tx, {
      orgId: session.orgId, actorUserId: session.userId,
      action: "distribution.create", entityType: "distribution", entityId: dist.id as string,
      details: { kind: "panel_invite", invited: panelists.length, excluded: exclusionSummary, seed: input.method === "random" ? seed : null },
    });
