    ...
    <div className="-m-4 min-h-[calc(100vh-3.5rem)] bg-background md:-m-6">
      <header className="sticky top-0 z-30 flex min-h-16 flex-wrap items-center gap-3 bg-transparent px-4 py-3 md:px-6">
        ...
        <a
          ...
          rel="noreferrer"
          aria-disabled={dirty}
          title={dirty ? "Save changes before sharing preview" : "Anonymous preview link expires after 24 hours"}
          onClick={(event) => { if (dirty) event.preventDefault(); }}
          className={cn(
            "inline-flex h-9 items-center rounded-md border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700",
            dirty ? "cursor-not-allowed opacity-50" : "hover:border-slate-300 hover:bg-slate-50",
          )}
        >
          Share preview
        </a>
    ...
    <article id={`question-${question.code}`} className={cn("scroll-mt-24 rounded-xl border p-4 transition-shadow focus-within:border-slate-400 focus-within:shadow-[0_4px_18px_rgba(15,23,42,0.08)]", question.hidden ? "border-dashed border-slate-300 bg-slate-50/80" : "border-[#d8dde1] bg-white")}>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <QuestionHeaderToolbar
          ...
          onLogicToggle={() => {
            if (hasLogic) {
              onChange({ visibleIf: undefined, visibleIfMode: undefined });
              setLogicOpen(false);
            } else {
              onChange({ visibleIf: [emptyDisplayCondition()] });
              setLogicOpen(true);
            }
          }}
...
function PrototypeQuestionBody({
  ...
  const config = question.prototype ?? {
    provider: "figma" as const,
    flowType: "task" as const,
    fileKey: "",
    startFrameId: "",
